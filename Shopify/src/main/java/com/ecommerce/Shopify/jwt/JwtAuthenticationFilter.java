...
import com.ecommerce.Shopify.service.CustomUserDetailsService;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (authorizationHeader == null || !authorizationHeader.startsWith("Bearer") || authorizationHeader.length() < 10) {
            sendException(response, request, HttpStatus.BAD_REQUEST.value(), "Access Denied ! No Authorization key in Header !");
            return;
        }
        ...
        try {
            ...
            if (!isValidToken) {
                sendException(response, request, HttpStatus.BAD_REQUEST.value(), "Access Denied ! Token is not valid !");
                return;
            }
            ...
            sendException(response, request, HttpStatus.BAD_REQUEST.value(), "Access Denied ! JWT signature does not match locally computed signature !");
            return;
        } catch (Exception e) {
